            ...
            <tbody className="divide-y divide-gray-50">
              {polizas?.map((p) => (
                <tr key={p.id} className="group hover:bg-ud-naranja-50/30 transition-colors cursor-pointer" onClick={() => navigate(`/polizas/${p.id}`)}>
                  ...
                  {/* AQUÍ ESTÁ TU BARRITA DE PROGRESO */}
                  <td className="px-4 py-3.5 w-24">
                    <div className="flex items-center gap-2">
                      <div className="w-full bg-gray-200 rounded-full h-1.5">
                        <div className="bg-blue-600 h-1.5 rounded-full" style={{ width: `${p.progreso_checklist || 0}%` }}></div>
                      </div>
                      <span className="text-[10px] font-bold text-gray-500">{p.progreso_checklist || 0}%</span>
                    </div>
                  </td>
