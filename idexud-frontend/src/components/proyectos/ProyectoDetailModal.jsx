...
export default function ProyectoDetailModal({ isOpen, onClose, proyecto }) {
  useEffect(() => {
    if (!isOpen) return;
  ...
  const contenido = (
    // Overlay — z-[9999] igual que PolizaModal para superponer cualquier elemento
    <div
    ...
    >
      {/*
        ...
        max-h-[90vh] + flex flex-col garantizan que NUNCA supere la pantalla.
        overflow-hidden en el contenedor evita que bordes redondeados se corten.
        El body toma flex-1 y hace scroll; header y footer son flex-shrink-0.
      */}
      <div
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="bg-gradient-to-br from-[#CC6628] to-[#a0511f] px-6 pt-5 pb-5 flex-shrink-0">
          ...
          {/* Badge código contable — mismo patrón que SolicitudDetailModal */}
        ...
        {/* ── CUERPO — flex-1 + overflow-y-auto (scroll interno) ───────── */}
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">

          {/* Valor vigente resaltado */}
          {proyecto.valor_vigente != null && (
            <Campo
              label="Valor Vigente del Contrato"
              valor={formatCOP(proyecto.valor_vigente)}
              resaltado
              full
            />
          )}
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-3">
            ...
            <Campo
              label="% Beneficio"
              valor={proyecto.pct_beneficio != null ? `${proyecto.pct_beneficio} %` : null}
            />
          </div>
          ...
          {/* ── Soporte documental (SECOP / Nextcloud) ───────────────── */}
              ...
              <div className="space-y-3">
                {proyecto.enlace_secop && (
                  <div>
                    ...
                    <Enlace href={proyecto.enlace_secop} label="Ver documento externo" />
                  </div>
                )}
                {proyecto.acto_administrativo && (
                  <div>
                    ...
                    {proyecto.acto_administrativo.startsWith('http') ? (
                      <Enlace href={proyecto.acto_administrativo} label={proyecto.acto_administrativo} />
                    ) : (
        ...
        {/* ── PIE — flex-shrink-0, siempre visible ──────────────────── */}
  ...
  // Portal al body — igual que PolizaModal, evita clipping por z-index o overflow de ancestros
